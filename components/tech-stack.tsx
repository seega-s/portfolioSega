...
import {
  ...
  siSpringboot, siCplusplus, siPython, siDocker, siGit, siPostgresql, siGnubash
} from 'simple-icons';
import { Bot, TerminalSquare, Network, Activity, Share2, Hexagon, Boxes, Award } from "lucide-react"
...
/* ─── SVG icon map for known tech names ─── */
const KNOWN_ICONS: Record<string, React.ReactNode> = {
  ...
  "spring boot": <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d={siSpringboot.path} /></svg>,
  "c++": <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d={siCplusplus.path} /></svg>,
  ...
  "bash": <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d={siGnubash.path} /></svg>,
  "gnu bash": <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d={siGnubash.path} /></svg>,
  "ai dev": <Bot size={18} strokeWidth={1.5} />,
  "prompt eng.": <TerminalSquare size={18} strokeWidth={1.5} />,
  "llm integration": <Network size={18} strokeWidth={1.5} />,
  "tcp/udp": <Activity size={18} strokeWidth={1.5} />,
  "p2p networks": <Share2 size={18} strokeWidth={1.5} />,
  "arch. hexagonal": <Hexagon size={18} strokeWidth={1.5} />,
  "ddd": <Boxes size={18} strokeWidth={1.5} />,
}

function getTechIcon(tech: TechItem): React.ReactNode {
  // First check if we have a known icon by name (case insensitive)
  const knownIcon = KNOWN_ICONS[tech.name.toLowerCase()]
  if (knownIcon) return knownIcon

  if (tech.icon_url) {
    return (
      <img src={tech.icon_url} alt={tech.name} className="w-full h-full object-contain filter group-hover:brightness-110 transition-all" />
    )
  }

  // Fallback
  return <Boxes size={18} strokeWidth={1.5} />
}
...
export function TechStack() {
  ...
  const [lists, setLists] = useState<TechList[]>([])
  ...
  return (
    <section className="w-full px-4 sm:px-6 py-12 sm:py-20 lg:px-12">
      {/* Section label */}
      <motion.div
      ...
      {!loading && certifications.length > 0 && (
        <div className="mb-14 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {certifications.map((cert, index) => (
            <motion.a
              ...
              className="group flex flex-col border border-teal/20 bg-transparent hover:border-teal transition-colors overflow-hidden"
            >
              <div className="aspect-video w-full bg-transparent flex items-center justify-center p-3 border-b border-teal/20">
                {cert.image_url ? (
                  <img src={cert.image_url} alt={cert.name_en} className="w-full h-full object-contain filter group-hover:brightness-110 transition-all" />
                ) : (
                  <Award className="w-8 h-8 text-teal/40 group-hover:scale-110 transition-transform" />
                )}
              </div>

              <div className="p-2.5 sm:p-3 flex flex-col gap-1 bg-teal/10">
                <span className="text-[10px] sm:text-xs font-bold tracking-tight text-foreground uppercase group-hover:text-teal transition-colors leading-tight line-clamp-2">
                  {lang === 'es' ? cert.name_es : cert.name_en}
                </span>
                <span className="text-[8px] sm:text-[10px] tracking-[0.1em] font-mono text-teal font-bold uppercase">
                  {cert.issuer}
                </span>
              </div>
            </motion.a>
        ...
        <div className="flex flex-col gap-6">
          {lists.map((category, catIndex) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-0">
                {(category.stack_techs || []).map((tech, techIndex) => (
                  <motion.div
                    key={tech.id}
                  ...
                  >
                    ...
                    <span className="text-xs font-mono text-foreground tracking-wide line-clamp-1">
                      {tech.name}
                    </span>
                  </motion.div>
                ))}
          ...
          {lists.length === 0 && (
            <div className="text-center py-12 text-muted-foreground font-mono text-sm border border-dashed border-border">
              No tech stack lists found. Please configure them in the admin panel.
            </div>
          )}
        </div>
      )}
